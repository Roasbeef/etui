...
-spec be_ok({ok, HXL} | {error, any()}) -> HXL.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, HXQ}) -> HXQ.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(HXT)) -> HXT.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
